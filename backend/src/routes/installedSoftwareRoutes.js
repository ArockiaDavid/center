...
router.post('/add-test-data/:userId', async (req, res) => {
  try {
    ...
    await InstalledSoftware.deleteMany({ user: req.params.userId });
...
router.get('/user/:userId', async (req, res) => {
  try {
    ...
    try {
      // Fetch software data
      const software = await InstalledSoftware.find({ user: new mongoose.Types.ObjectId(req.params.userId) })
      ...
      if (software.length === 0) {
        console.log('No software found, creating test data...');
        const testSoftware = [
          {
        ...
        const createdSoftware = await InstalledSoftware.insertMany(testSoftware);
        console.log('Test data created:', createdSoftware);

        // Transform dates to ISO strings for consistent formatting
        const formattedSoftware = createdSoftware.map(item => ({
          ...item.toObject(),
        ...
        console.log('Sending formatted test software:', formattedSoftware);
        return res.json(formattedSoftware);
      }
...
router.delete('/:appId', async (req, res) => {
  try {
    const result = await InstalledSoftware.findOneAndDelete({
      user: req.user._id,
      appId: req.params.appId
    });
